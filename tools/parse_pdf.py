...
def day_from_fragments(fragments):
    """День по кускам вертикальной подписи, разорванной между страницами.

    «Среда» на границе страниц приходит как «а» внизу одной и «дерС» вверху
    следующей. Ни один кусок сам по себе днём не читается, и раньше такой
    день молча брался у следующего блока — среда уезжала в четверг.
    """
    parts = [f[::-1].lower() for f in fragments if f]
    for joined in ("".join(parts), "".join(reversed(parts))):
        if joined in DAYS:
            return DAYS[joined]
    # Кусок из трёх и больше букв, однозначно начинающий или кончающий день.
    for part in parts:
        if len(part) >= 3:
            hits = {n for name, n in DAYS.items() if name.startswith(part) or name.endswith(part)}
            if len(hits) == 1:
                return hits.pop()
    return None
...
# Скобка со временем («(08.00-10.40)») пометкой не считается: это начало
# следующей пары, иначе время уехало бы в примечание предыдущей.
RE_TEACHER_END = re.compile(
    r"[А-ЯЁ]\.\s*(?:[А-ЯЁ]\.?)?(?![а-яё])"
    r"(?:\s*\((?!\s*\d{1,2}[.:]\d{2}\s*[-–—])[^)]*\))?"
)

# Ссылка на встречу после пары: «Войтова Х.А. (1):⏎https://telemost…».
# Номер с двоеточием — подпись ссылки, а не пометка. Ссылка бывает разорвана
# переносом, поэтому хвост — ещё куски без кириллицы, похожие на адрес.
RE_LINK_AFTER = re.compile(
    r"\s*(?:\(\d+\))?\s*:?\s*https?://\S+"
    r"(?:\s+(?=[^\sА-Яа-яЁё(]*[\d/=?&%._-])[^\sА-Яа-яЁё(]+)*"
)
...
def parse_lesson(chunk):
    ...
    if url:
        ...
        # «(1):» перед ссылкой — её подпись, а не пометка к паре.
...
def split_lessons(cell):
    ...
    for i, mark in enumerate(marks):
        ...
        # Ссылка сразу после пары — её часть. Иначе ссылка вместе со всем
        # хвостом ячейки уходила в начало следующей пары: та теряла название,
        # а в расписании появлялась пара «:».
        link = RE_LINK_AFTER.match(text, end)
        if link:
            end = link.end()
            ...
            for index, cell_row in enumerate(table.rows):
                ...
                if reversed_first in DAYS:
                    ...
                    if block["rows"] and block["day"] not in (None, day):
                        ...
                        block["day"] = day
                elif first and blocks[-1]["day"] is None:
                    # Кусок подписи, разорванной границей страниц.
                    block = blocks[-1]
                    block.setdefault("fragments", []).append(first)
                    block["day"] = day_from_fragments(block["fragments"])
